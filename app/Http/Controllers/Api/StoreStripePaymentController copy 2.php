...
class StoreStripePaymentController extends Controller
{
    ...
    public function createOrder(Request $request)
    {
        try {
            ...
            // Create a fresh PaymentIntent for every checkout attempt.
            // The previous deterministic key could reuse a terminal PaymentIntent.
            $idempotencyKey = $request->header('Idempotency-Key') ?: 'store_pi_' . Str::uuid();
        ...
        try {
            ...
            if ($finalAmount > 0) {
                ...
                try {
                    ...
                    Log::error('STRIPE PAYMENT INTENT RETRIEVE ERROR', [
                        'payment_intent_id' => $request->payment_intent_id,
                        'error' => $stripeException->getMessage(),
                    ]);

                    throw new \Exception('Payment could not be verified. Please try again.');
                }
                ...
                if ((int) $intent->amount !== $expectedAmountInCents) {
                    ...
                    throw new \Exception('Payment amount could not be verified. Please try again.');
                }
                ...
                if (strtolower($intent->currency) !== 'usd') {
                    ...
                    throw new \Exception('Payment currency could not be verified. Please try again.');
                }
                ...
                if ($intent->payment_method) {
                    try {
                        ...
                        Log::warning('STRIPE PAYMENT METHOD RETRIEVE ERROR', [
                            'payment_intent_id' => $intent->id,
