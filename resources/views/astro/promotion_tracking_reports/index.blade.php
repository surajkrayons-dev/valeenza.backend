    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">All Promotion Tracking Reports</h4>
                <div class="page-title-right">
                    <a href="{{ route('client.promotion_tracking_reports.export.xlsx.data') }}"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col">
                            <div class="form-group">
                                <label for="promotion_product_id" class="form-label">Promotion Product :</label>
                                <select name="promotion_product_id" id="promotion_product_id"
                                    class="form-control select2-class2" data-placeholder="Choose Promotion Product">
                                    <option value="">Choose Promotion Product</option>
                                    @foreach ($promotionProducts as $promotionProduct)
                                        <option value="{{ $promotionProduct->id }}">
                                            [{{ $promotionProduct->code }}]-{{ $promotionProduct->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $(document).on('click', '#reset-filter-btn', function(e) {
                ...
                $('#promotion_product_id').val('').trigger('change');
            ...
            $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('client.promotion_tracking_reports.list') }}',
                    data: function(d) {
                        d.store_id = $('#store_id').val();
                        d.promoter_id = $('#promoter_id').val();
                        d.promotion_product_id = $('#promotion_product_id').val();
                ...
                columns: [{
                    ...
                    {
                        data: 'promotion_product_info',
                        name: 'promotion_product_info'
                    },
                    {
                        data: null,
                        ...
                        mRender: (data, type, row) => {
                            return `
                                <a href="javascript:void(0);" data-href="{{ route('client.promotion_tracking_reports.view') }}/${row.id}" class="btn btn-soft-success btn-sm waves-effect waves-light open-remote-modal" data-target="#remoteModal"><i class="mdi mdi-eye font-size-16"></i></a>
                            `;
                        },
                        orderable: false,
                        searchable: false
            ...
            $('#store_id, #promoter_id, #promotion_product_id').on('input change', function() {
                reloadTable('#data-table');
            });
