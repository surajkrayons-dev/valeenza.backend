    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Store Banners</h4>

                <div class="page-title-right">
                    <a href="{{ route('admin.store_banners.create.index') }}" class="btn btn-soft-info">
    ...
    <script>
        $(function() {
            ...
            let table = $('#data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.store_banners.list') }}",
                    data: function(d) {
                        d.status = $('#status').val();
                    }
                },
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <a href="{{ route('admin.store_banners.update.index') }}/${row.id}"
                                class="btn btn-soft-info">
                                    <i class="bx bx-pencil"></i>
                                </a>
                                <button class="btn btn-soft-danger delete-entry"
                                        data-href="{{ route('admin.store_banners.delete') }}/${row.id}">
            ...
            $(document).on('change', '.change-status', function() {
                ...
                $.get(`{{ route('admin.store_banners.change.status') }}/${id}`)
                    .done(function() {
