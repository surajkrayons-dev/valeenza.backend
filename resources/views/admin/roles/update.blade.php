                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="name" name="role_name" class="form-control" placeholder="Enter Role Name" value="{{ $role->name }}" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <table id="" class="table table-bordered nowrap w-100">
                                            ...
                                            <tbody>
                                                @foreach (config('permissions.permissions_names') as $key => $text)
                                                    @php $actions = config("permissions.available_permissions.{$key}"); @endphp
                                                    <tr>
                                                        <td>{{ $text }}</td>
                                                            ...
                                                            @if (in_array($action, $actions))
                                                                <td>
                                                                    <div class="square-switch">
                                                                        <input type="checkbox" id="permission-switch-{{ $key }}-{{ $action }}" switch="status" value="1" name="permissions[{{ $key }}][{{ $action }}]" {{ isset($role->permissions[$key]) && in_array($action, $role->permissions[$key]) ? 'checked' : ''  }} />
