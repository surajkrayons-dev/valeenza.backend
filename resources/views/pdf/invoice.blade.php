...
<style>
    * { box-sizing: border-box; }
    ...
    .header-table .logo-cell img { height: 40px; }
    .title { text-align: right; }
    .title h2 { margin: 0; font-size: 16px; }
    .title p { margin: 2px 0 0; font-size: 11px; color: #666; }

    /* SOLD BY / ADDRESS */
    .top-table td { vertical-align: top; padding: 0; font-size: 11px; line-height: 1.6; }
    .top-table .right-col { text-align: right; }
    .top-table strong { display: block; margin-bottom: 3px; }

    /* META */
    .meta-table td { font-size: 11px; padding: 4px 0; vertical-align: top; }
    .meta-table .right-col { text-align: right; }

    /* ITEMS */
    table.items { margin-top: 15px; font-size: 11px; }
    ...
    table.items td.desc small { color: #666; display: block; }
    .total-row td { font-weight: bold; text-align: right !important; background: #f2f2f2; }

    /* INFO BOX */
    table.info-box { border: 1px solid #999; border-top: none; }
    table.info-box td { padding: 12px; font-size: 11px; vertical-align: top; }
    table.info-box .sign-col { text-align: right; width: 220px; font-weight: bold; }
    table.info-box .sign-col img.signature { height: 40px; display: block; margin: 6px 0 4px auto; }

    .footer-note { border: 1px solid #999; border-top: none; padding: 6px 8px; font-size: 11px; }
...
<body>
        ...
        <tr>
            ...
            <td width="50%" class="right-col">
                ...
                @if(!$sameAddress)
                    ...
                    {{ $order->addressData->city }}, {{ $order->addressData->state }}, {{ $order->addressData->country }} - {{ $order->addressData->pincode }},<br>
    ...
    <table class="items">
        ...
        <tbody>
            ...
            {{-- @if($order->price_breakdown['cod_charge'] ?? 0)
            <tr>
                <td></td>
                <td class="desc">COD Charge</td>
                <td>${{ number_format($order->price_breakdown['cod_charge'], 2) }}</td>
                <td>1</td>
                <td>${{ number_format($order->price_breakdown['cod_taxable_amount'] ?? 0, 2) }}</td>
                <td>{{ $order->price_breakdown['cod_gst_rate'] ?? 18 }}%</td>
                <td>{{ $order->tax_type }}</td>
                <td>${{ number_format($order->price_breakdown['cod_gst_amount'] ?? 0, 2) }}</td>
                <td>${{ number_format($order->price_breakdown['cod_charge'], 2) }}</td>
            </tr>
            @endif --}}
